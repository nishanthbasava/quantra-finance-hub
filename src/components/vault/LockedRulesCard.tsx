import { Lock, ExternalLink, Plus } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import type { VaultRule } from "@/lib/xrplVault/types";

interface Props {
  rules: VaultRule[];
  onHistory: (rule: VaultRule) => void;
  onCreateNew: () => void;
}

const LockedRulesCard = ({ rules, onHistory, onCreateNew }: Props) => (
  <div className="quantra-card p-0 overflow-hidden">
    ...
    <div className="divide-y divide-border/50">
      {rules.map((rule) => {
        const dateStr = new Date(rule.createdAt).toLocaleDateString("en-US", {
          month: "short",
          day: "numeric",
          year: "numeric",
        });
